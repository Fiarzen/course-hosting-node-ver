...
async function canViewFullLessonContent(user: any | null, courseId: number) {
  ...
  const course = await prisma.course.findUnique({ where: { id: courseId } });
  if (!course) return false;
  if (isAdmin(user) || (course.authorId && course.authorId === user.id)) return true;
...
lessonsRouter.get("/", async (req: AuthenticatedRequest, res) => {
  ...
  const authoredCourses = await prisma.course.findMany({ where: { authorId: user.id } });
...
lessonsRouter.get("/course/:courseId", async (req: AuthenticatedRequest, res) => {
  const user = req.user || null;
  if (!user) return res.status(401).json({ error: "Not authenticated" });
...
lessonsRouter.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    if (!course) return res.status(400).json({ error: `Course not found with id: ${cid}` });
...
lessonsRouter.post("/course/:courseId/reorder", async (req: AuthenticatedRequest, res) => {
  const user = req.user || null;
  if (!user) return res.status(401).json({ error: "Not authenticated" });
  ...
  for (const id of orderedLessonIds) {
    ...
    if (lesson) {
      await prisma.lesson.update({ where: { id }, data: { orderIndex: index++ } });
      byId.delete(id);
    }
  }
